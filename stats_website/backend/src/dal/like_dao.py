...
class LikeDAO(BaseDAO):
    ...
    def count_total(self) -> int:
        """Count total number of likes in the database."""
        with self._cursor() as cur:
            cur.execute("SELECT COUNT(*) as count FROM likes")
            result = cur.fetchone()
            return result["count"] if result else 0

    def get_distribution_by_destination(self) -> list[dict[str, any]]:
        """Get likes distribution grouped by vacation destination.
        
        Returns:
            List of dicts with 'destination' and 'likes' keys, sorted by likes descending
        """
        with self._cursor() as cur:
            cur.execute(
                """SELECT 
